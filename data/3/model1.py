...
class CustomModelProcessorDataset:
    def __init__(self, config_path):
        ...
        self.masked_data = pd.read_csv(self.masked_path)
...
def main():
    parser = argparse.ArgumentParser(description='Custom Model for Dataset 3')
    parser.add_argument('--config', type=str, required=True, help='Path to config.yaml')
    args = parser.parse_args()
    processor = CustomModelProcessorDataset(args.config)
    processor.process()
